 ...
 */
export function getCreatureStats(assetId) {
 ...
 */
export function getAssetComments(assetId, startIndex = 0, length = 1000) {
    return get(`/rest/comments/${assetId}/${startIndex}/${length}`);
}
/**
 * Get the XML sporemodel file for an asset
 */
export function getXmlModel(assetId) {
 ...
 */
export function getSmallPngUrl(assetId) {
    const id = assetId.toString();
 ...
 */
export function getLargePngUrl(assetId) {
    const id = assetId.toString();

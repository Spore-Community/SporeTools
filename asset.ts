...
const ASSET_ID_INPUT = document.getElementById("AssetIdInput") as HTMLInputElement;
...
const PNG_INPUT = document.getElementById("PngInput") as HTMLInputElement;
 ...
 */
function updateAssetCard(name: string, author: string, smallPngUrl: string | undefined, largePngUrl: string | undefined, id: number | undefined) {
    // Main data
    ...
    // PNGs
    const smallCanvas = (document.getElementById("SmallPng") as HTMLCanvasElement).getContext("2d");
    ...
    const largeCanvas = (document.getElementById("LargePng") as HTMLCanvasElement).getContext("2d");
 ...
 */
async function updateServerData(assetId: number | undefined, analyzePng = true) {
    if (assetId && assetId > 0) {
        ...
        parentIds.reverse().forEach(parentId => {
            ...
            document.getElementById("ServerLineageData")!.innerHTML += `<div class="lineageAsset"><a href="?assetId=${parentId}"><img src="${pngUrl}" width="128" height="128"></a><pre>${parentId}</pre></div>`;
        });
 ...
 */
function updatePngData(png: ArrayBuffer) {
...
document.getElementById("CopyAssetId")!.addEventListener("click", () => {
    const ASSET_ID = document.getElementById("AssetId") as HTMLParagraphElement;
